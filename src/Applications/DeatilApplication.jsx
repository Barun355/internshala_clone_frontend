...
useEffect(()=>{
  ...
  const response=await axios.get(`https://internshala-clone-backend-uits.onrender.com/api/application/${id}`)
...
const handleAcceptAndReject= async(id,action)=>{
  try {
    const response=await axios.put(`https://internshala-clone-backend-uits.onrender.com/api/application/${id}`,{action})
