...
function UserApplicatiom() {
    ...
    useEffect(()=>{
        ...
        try {
            const response=await axios.get(`${process.env.REACT_APP_BACKEND_API}/application`)
...
<tbody>
    {
        userapplication.map((data)=>(
            ...
            <td className='whitespace-nowrap px-6 py-4'><Link to={`/UserapplicationDetail?a=${data._id}`}><i className="bi bi-envelope-open text-blue-500"></i></Link></td>
       ...
       <div className="p-4 lg:w-1/2 md:w-full">
         <div className="flex border-2 rounded-lg border-gray-200 border-opacity-50 p-8 sm:flex-row flex-col">
           ...
           <div className="flex-grow">
             ...
             <Link to={`/UserapplicationDetail?a=${data._id}`} className="mt-3 text-indigo-500 inline-flex items-center">View in deatil
